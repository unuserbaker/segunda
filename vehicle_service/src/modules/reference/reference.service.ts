...
export class ReferenceService {
  ...
  async findAllBrands(page = 1, size = 10) {
    return this.paginate(this.brandRepo, page, size);
  }

  async findAllCategories(page = 1, size = 10) {
    return this.paginate(this.categoryRepo, page, size);
  }

  async findAllEngineTypes(page = 1, size = 10) {
    return this.paginate(this.engineTypeRepo, page, size);
  }

  async findAllTransmissions(page = 1, size = 10) {
    return this.paginate(this.transmissionRepo, page, size);
  }

  async findAllTypes(page = 1, size = 10) {
    return this.paginate(this.typeRepo, page, size);
  }

  async findAllStatuses(page = 1, size = 10) {
    return this.paginate(this.statusRepo, page, size);
  }
